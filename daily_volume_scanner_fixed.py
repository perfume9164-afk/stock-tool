...
# 主要銘柄の日本語社名。yfinanceの英語名をそのまま保存しないためのマスタ。
JP_NAMES = {
    "7203.T":"トヨタ自動車", "7269.T":"スズキ", "7267.T":"本田技研工業",
    "5020.T":"ENEOSホールディングス", "8604.T":"野村ホールディングス",
    "8750.T":"第一生命ホールディングス", "4901.T":"富士フイルムホールディングス",
    "8309.T":"三井住友トラストグループ", "8306.T":"三菱UFJフィナンシャル・グループ",
    "9432.T":"NTT", "6758.T":"ソニーグループ", "9984.T":"ソフトバンクグループ",
    "6702.T":"富士通", "8035.T":"東京エレクトロン", "4063.T":"信越化学工業",
    "4519.T":"中外製薬", "4188.T":"三菱ケミカルグループ", "6315.T":"TOWA",
    "6526.T":"ソシオネクスト", "5803.T":"フジクラ", "7013.T":"IHI",
    "4661.T":"オリエンタルランド", "6367.T":"ダイキン工業", "2914.T":"日本たばこ産業",
    "9983.T":"ファーストリテイリング", "6098.T":"リクルートホールディングス",
    "8001.T":"伊藤忠商事", "8002.T":"丸紅", "8058.T":"三菱商事", "8031.T":"三井物産",
    "8411.T":"みずほフィナンシャルグループ", "8316.T":"三井住友フィナンシャルグループ",
    "8418.T":"山口フィナンシャルグループ", "8601.T":"大和証券グループ本社",
    "8801.T":"三井不動産", "8802.T":"三菱地所", "9020.T":"東日本旅客鉄道",
    "9021.T":"西日本旅客鉄道", "9022.T":"東海旅客鉄道", "9005.T":"東急",
    "9433.T":"KDDI", "9434.T":"ソフトバンク", "9501.T":"東京電力ホールディングス",
    "9502.T":"中部電力", "9503.T":"関西電力", "9101.T":"日本郵船", "9104.T":"商船三井",
    "9107.T":"川崎汽船", "5401.T":"日本製鉄", "5711.T":"三菱マテリアル",
    "5713.T":"住友金属鉱山", "6762.T":"TDK", "6857.T":"アドバンテスト",
    "6723.T":"ルネサスエレクトロニクス", "6146.T":"ディスコ", "7735.T":"SCREENホールディングス",
    "7741.T":"HOYA", "6971.T":"京セラ", "7751.T":"キヤノン", "6501.T":"日立製作所",
    "6503.T":"三菱電機", "6504.T":"富士電機", "7011.T":"三菱重工業",
    "7201.T":"日産自動車", "7202.T":"いすゞ自動車", "7205.T":"日野自動車",
    "7211.T":"三菱自動車工業", "7270.T":"SUBARU", "7282.T":"豊田合成",
    "7272.T":"ヤマハ発動機", "7261.T":"マツダ", "7453.T":"良品計画",
    "3382.T":"セブン＆アイ・ホールディングス", "8267.T":"イオン", "2914.T":"日本たばこ産業",
}

def company_name(ticker, yname=""):
    return JP_NAMES.get(ticker) or (yname if any("\u3040" <= c <= "\u30ff" or "\u4e00" <= c <= "\u9fff" for c in str(yname)) else ticker)

def fallback_reason(x):
    fa=int(x.get("funda_score") or 0); ch=int(x.get("chart_score") or 0); total=int(x.get("total_score") or 0)
    vr=float(x.get("volume_ratio") or 1); rsi=x.get("rsi")
    f="ファンダメンタルズが比較的強い" if fa>=40 else "ファンダメンタルズはまずまず" if fa>=32 else "ファンダメンタルズは慎重に確認"
    c="チャートは強含み" if ch>=38 else "チャートは中立〜やや強含み" if ch>=30 else "チャートはまだ慎重"
    r=f"RSI {float(rsi):.1f}" if rsi is not None else "RSI確認"
    return f"総合{total}/100点。{f}。{c}。出来高は平均比{vr:.2f}倍、{r}。"
    ...
    for rank, (ticker, volume, close, ratio) in enumerate(top50, 1):
        try:
            ...
            out.append({
                ...
                "name": company_name(ticker, info.get("longName") or info.get("shortName") or ticker),
...
def main():
    ...
    if not ai:
        ...
        ai = [dict(x, ai_rank=i+1, reason=fallback_reason(x),
